...
const { isLoading, isLoadingMoreStarships, starshipArray, currentPage } = storeToRefs(store);
...
<template>
  ...
  <ul v-else class="starship-list-ul">
    <li class="starship-list-li" v-for="{ name, model, url } in starshipArray" :key="name">
